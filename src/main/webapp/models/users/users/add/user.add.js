...
function UserAddCtrl($scope, UserService, $http, $filter, $state,  FileUploadService) {
        ...
        service.getPosts(departmentId)
            .then(function (response) {
                processServerResponse(response.data, null, function (data) {
                    $scope.departmentPosts[index] = data;
                    var admin = $filter('filter')($scope.departmentPosts[index], {name: 'Администратор'});
        ...
        service.add()
            .then(function (response) {
                processServerResponse(response.data, null, function (data) {
                    ...
                    $scope.userForm.serviceInformation = [];
                    $scope.userForm.userCategories = [];
                    $scope.userForm.educations = [];
                    $scope.userForm.certifications = [];
                    $scope.userForm.userJobs = [];

                    $scope.addServiceInformation();
                    $scope.addCategory();
                    $scope.addEducation();
    ...
    $scope.saveUser = function () {
        ...
        if ($scope.valid) {
            ...
            $scope.userForm.filename = photo != null ? photo.name : '';
