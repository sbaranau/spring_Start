...
function paginationXDirective($filter, $compile) {
	return {
		...
		templateUrl: 'js/paginationx/paginationx.htm',
		link: linkFn
	};
	...
	function linkFn(scope, elem, attrs) {
		...
		var getTHeader = function(columns) {
			...
			for (var j = 0; j < columns.length; j++) {
				tableHeader = tableHeader + '<th class="pagination-header" width="' + columns[j].width + '" style="' + columns[j].style + '"> <div class="pagination-header-link" ng-click="sort(\'' + columns[j].sortKey + '\')" translate="' + columns[j].title+ '">';
		...
		var getTBody = function(columns) {
			...
			for (var i = 0; i < scope.page.length; i++) {
				...
				for (var j = 0; j < columns.length; j++) {
					...
					var dataOfColumn = prepareDataOfColumn(obj[columns[j].dataKey] ? obj[columns[j].dataKey] : '', columns[j], i);
					tableBody = tableBody + '<td width="' + columns[j].width + '" style="' + columnStyle + '" class="' + cellStyle + '">' + dataOfColumn + '</td>';
				}
				if (scope.features.actionColumn) {
		...
		var getActionColumn = function(rowActions, index) {
			...
			for (var i = 0; i < actions.length; i++) {
                var conditions = checkConditionsInActionsColumn(actions[i], index);
                var status = actions[i].status ? actions[i].status : '';
                var label = actions[i].label ? actions[i].label : '';
				if (actions[i].type === undefined || actions[i].type === "Button") {
					actionColumn = actionColumn + '<button class="btn btn-primary" ' + conditions + actions[i].htmlAttrbs + ' ng-click="actionHandler(\'' + actions[i].name + '\',page[' + index + '])" ' + status + ' translate="'+label+'"></button>&nbsp';
				} else if (actions[i].type === "Link") {
                    var href = actions[i].href ? ' href="' + actions[i].href + '" ': '';
					actionColumn = actionColumn + '<a ' + conditions + actions[i].htmlAttrbs + href + 'ng-click="actionHandler(\'' + actions[i].name + '\',page[' + index + '])" ' + status + ' translate="'+label+'"></a>&nbsp';
				}
			}
        ...
        /**Действия в columns на данных внутри таблицы*/
        function prepareDataOfColumn(data, column, index) {
        	if (!!data) {
                /**when we get date or boolean(need to check mark)*/
                if (column.dateColumn) {
                    data = column.dateColumn.withTime ? '{{' + data + '| date: \'dd.MM.yyyy HH:mm\'}}' : '{{' + data + '| date: \'dd.MM.yyyy\'}}';
                } else if (column.dateColumn) {
                    data = '';
                } else if (column.booleanColumn) {
                    data = data ? '<span class="checked-icon" >&nbsp;</span>': '';
                }
                /**when we get SimpleReferencesDTO object instead string*/
                if (column.isRefData) {
                    data = data.name;
                }
            }
            ...
            if (column.actions) {
            	switch (column.actions.type) {
					case 'link':
						if (column.actions.param) {
                            if (column.actions.param.getDataFromField) {
                                data = '<a href="' + column.actions.action.replace("?param?", "{{page["+index+"][\'"+column.actions.param.getDataFromField+"\']}}") + '">' + data + '</a>';
							} else {
                                data = '<a href="' + column.actions.param ? column.actions.action.replace("?param?", column.actions.param) : column.actions.action + '">' + data + '</a>';
							}
						}
                        break;
					case 'uiLink':
                        if (column.actions.param) {
                            if (column.actions.param.getDataFromField) {
                                data = '<a ui-sref="' + column.actions.action.replace("?param?", "{{page["+index+"][\'"+column.actions.param.getDataFromField+"\']}}") + '">' + data + '</a>';
                            } else {
                                data = '<a ui-sref="' + column.actions.param ? column.actions.action.replace("?param?", column.actions.param) : column.actions.action + '">' + data + '</a>';
                            }
                        }
						break;
					case 'ngAction':
						data = '<a ng-click="actionHandler(\'' + column.actions.action + '\', page[' + index + '])">' + data + '</a>';
						break;
					default:
						break;
				}
        ...
        /**Действие ng-disabled на кнопке из actions колонки*/
        function checkConditionsInActionsColumn(actionCell, index) {
        	var data = '';
            if (actionCell.conditions) {
            	if (actionCell.conditions.action === 'exist') {
            		switch (actionCell.conditions.type) {
						case "ngDisabled":
							data = ' ng-disabled="!page[' + index + '][\''+actionCell.conditions.field+'\']" ';
							break;
						case "ngShow":
                            data = ' ng-show="!!page[' + index + '][\''+actionCell.conditions.field+'\']" ';
                            break;
						case "ngIf":
                            data = ' ng-if="!!page[' + index + '][\''+actionCell.conditions.field+'\']" ';
                            break;
						default:
                            break;
					}
