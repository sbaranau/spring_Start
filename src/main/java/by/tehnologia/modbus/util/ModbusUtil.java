package by.tehnologia.modbus.util;

/**
 * @author Aleksandr Golovnyov
 */
public final class ModbusUtil {

    private ModbusUtil() { }

    /**
     * Converts the given register (16-bit value) into
     * a <tt>int</tt>.
     * The value returned is:
     * <p/>
     * <pre><code>
     * ((a &lt;&lt; 8) | (b &amp; 0xff))
     * </code></pre>
     * <p/>
     * This conversion has been taken from the documentation of
     * the <tt>DataInput</tt> interface.
     *
     * @param bytes bytes a register as <tt>byte[2]</tt>.
     * @return the signed short as <tt>short</tt>.
     */
    public static int getRegister(byte[] bytes) {
        return getRegister(bytes, 0);
    }

    /**
     * Converts the given register (16-bit value) into
     * a <tt>int</tt>.
     * The value returned is:
     * <p/>
     * <pre><code>
     * ((a &lt;&lt; 8) | (b &amp; 0xff))
     * </code></pre>
     * <p/>
     * This conversion has been taken from the documentation of
     * the <tt>DataInput</tt> interface.
     *
     * @see ModbusUtil#registerToShort
     * @param bytes bytes bytes a register as <tt>byte[2]</tt>.
     * @param offset offset
     * @return
     */
    public static int getRegister(byte[] bytes, int offset) {
        return ((bytes[offset] << 8) | (bytes[offset + 1] & 0xff));
    }

    public static byte lowByte(int wd) {
        return (byte) (0xff & wd);
    }

    public static byte hiByte(int wd) {
        return (byte) (0xff & (wd >> 8));
    }

    public static int[] calculateCRC(byte[] data, int offset, int len) {

        int[] crc = {0xFF, 0xFF};
        int nextByte = 0;
        int uIndex; /* will index into CRC lookup*/ /* table */
        /* pass through message buffer */
        for (int i = offset; i < len && i < data.length; i++) {
          nextByte = 0xFF & ((int) data[i]);
          uIndex = crc[0] ^ nextByte; //*puchMsg++; /* calculate the CRC */
          crc[0] = crc[1] ^ CRC_HI[uIndex];
          crc[1] = CRC_LO[uIndex];
        }

        return crc;
    }

    /* Table of CRC values for high-order byte */
    private static final short[] CRC_HI = {
      
    };

    /* Table of CRC values for low-order byte */
    private static final short[] CRC_LO = {
      
    };
}
